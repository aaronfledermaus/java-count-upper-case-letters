...
`git clone https://github.com/ByteLegendQuest/java-count-upper-case-letters`

A project directory named `java-count-upper-case-letters` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-count-upper-case-letters` directory which is created just now.
